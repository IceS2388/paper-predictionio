...
**评估模板**
```shell
$ sbt clean package && pio-docker eval org.example.recommendation.RecommendationEvaluation org.example.recommendation.EngineParamsList -- --driver-memory 4g --executor-memory 5g --verbose
```
...
### 7.1 基于用户自身历史记录的推荐。
* 根据电影的类型来提取。
用户自身观影类型偏好：(类型1：电影1的评分+...+电影n的评分，类型2,：电影1的评分+...+电影n的评分)
* 电影演员特征
用户自身演员偏好：(类型1：电影1的评分+...+电影n的评分，类型2,：电影1的评分+...+电影n的评分)
